...
final class ModalStylesViewController: CollectionViewController
{
    convenience init() {
        let sections = [
            Section(title: "UIModalPresentationStyle", rows: type(of: self).createRows()),
            Section(model: HeaderModel(title: "UIModalPresentationStyle"), rows: type(of: self).createRows())
        ]
